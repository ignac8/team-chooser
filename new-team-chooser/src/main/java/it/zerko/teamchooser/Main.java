...
//      new Player("Walo", 1)
    ...
    Combination bestCombination = CombinatoricsFactory.createSimpleCombinationGenerator(
      ...
      .filter(Main::firstTeamWithIgnac)
  ...
  private static boolean firstTeamWithIgnac(Combination combination) {
    return combination.getFirstTeam().getPlayers().stream().anyMatch(player -> player.getNickname().equals("ignac8"));
  }
